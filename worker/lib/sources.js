...
export async function addApprovedSource(url, env) {
  try {
    ...
    const exists = sources.approved.some(s => 
      (typeof s === 'string' ? s : s.url) === url
    );
...
export async function addCandidateSource(url, title, env) {
  try {
    ...
    const inApproved = sources.approved.some(s => 
      (typeof s === 'string' ? s : s.url) === url
    );
    const inCandidates = sources.candidates.some(c => 
      (typeof c === 'string' ? c : c.url) === url
    );
