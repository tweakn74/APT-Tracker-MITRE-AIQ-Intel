...
test.describe('Dashboard 5: Dark Theme Enhancements', () => {
  ...
  test('Executive Metrics Landing Page uses Versedetect color scheme', async ({ page }) => {
    ...
    // Check metric card background
    const metricCard = page.locator('.metric-card').first();
    await expect(metricCard).toHaveCSS('background', /rgb\(21, 24, 33\)/); // #151821
  ...
  test('Border radius values are consistent across pages', async ({ page }) => {
    ...
    const metricCard = page.locator('.metric-card').first();
    const borderRadius = await metricCard.evaluate(el => getComputedStyle(el).borderRadius);
    expect(borderRadius).toMatch(/12px|16px|22px/); // --radius-sm, --radius-md, or --radius-lg
  ...
  test('Shadow values are consistent across pages', async ({ page }) => {
    ...
    const metricCard = page.locator('.metric-card').first();
    const boxShadow = await metricCard.evaluate(el => getComputedStyle(el).boxShadow);
  ...
  test('Hover effects work with new color scheme', async ({ page }) => {
    ...
    const transform = await metricCard.evaluate(el => getComputedStyle(el).transform);
